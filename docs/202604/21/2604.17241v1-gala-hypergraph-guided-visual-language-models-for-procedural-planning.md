...
tags: ["query:vlm-rl"]
score: 8.0
evidence: 超图引导的视觉语言模型用于多模态过程规划与空间推理
tldr: 针对具身智能中程序化规划任务，现有视觉语言模型（VLM）难以有效捕捉物体间的隐式空间关系和深层语义结构。本文提出GaLa框架，通过超图表示法将物体建模为节点，并根据属性和功能语义构建区域级超边。结合TriView超图编码器，利用对比学习增强语义一致性，显著提升了模型在ActPlan1K和ALFRED基准上的规划成功率和准确性。
selection_source: fresh_fetch
motivation: 现有VLM过度依赖自身推理能力，忽视了多模态输入中丰富的结构化语义信息，导致难以理解复杂场景中的功能空间关系。
method: 提出GaLa框架，利用超图显式建模物体间的隐式语义关系，并通过TriView编码器在节点、区域及关联视图间强制执行语义一致性。
result: 在ActPlan1K和ALFRED基准测试中，GaLa在执行成功率、LCS和规划正确性等指标上均显著优于现有方法。
conclusion: 通过引入超图引导的结构化语义表示，可以有效增强视觉语言模型在复杂具身场景下的程序化规划能力。
---

## 摘要
物体属性中编码的隐式空间关系和深层语义结构对于具身智能系统中的过程规划至关重要。然而，现有方法往往过度依赖视觉语言模型（VLMs）自身的推理能力，而忽视了可以从多模态输入中挖掘的丰富结构化语义信息。因此，模型难以有效理解复杂场景中的功能性空间关系。为了充分利用多模态数据中的隐式空间关系和深层语义结构，我们提出了 GaLa，这是一个用于多模态过程规划的视觉语言框架。GaLa 引入了一种基于超图的表示方法，将图像中的物体实例建模为节点，并根据物体的属性和功能语义聚合物体，从而构建区域级超边。该设计显式地捕捉了物体间的隐式语义关系以及功能区域的层级组织。此外，我们设计了一个三视图超图编码器（TriView HyperGraph Encoder），通过对比学习强制节点视图、区域视图和节点-区域关联视图之间的语义一致性，使超图语义能够更有效地注入到下游 VLM 推理中。在 ActPlan1K 和 ALFRED 基准测试上的广泛实验表明，GaLa 在执行成功率、LCS 和规划正确性方面显著优于现有方法。

...
tags: ["query:vlm-rl"]
score: 6.0
evidence: 用于驾驶智能体视觉语言理解与推理的VLM
tldr: LMGenDrive是首个将大语言模型（LLM）的多模态理解能力与生成式世界模型相结合的端到端闭环自动驾驶框架。针对自动驾驶在长尾和开放场景中泛化性差的挑战，该模型通过多视角相机和自然语言指令，同步生成未来驾驶视频与控制信号。利用LLM的语义先验和世界模型的时空建模能力，配合三阶段渐进式训练策略，LMGenDrive在复杂环境下的决策鲁棒性和指令遵循能力显著提升。
selection_source: fresh_fetch
motivation: 旨在解决自动驾驶在长尾和开放世界场景中泛化能力不足的问题，通过统一多模态理解与生成式想象来增强决策的鲁棒性。
method: 提出了一个集成LLM与世界模型的统一框架，通过三阶段渐进式训练实现多视角视频预测与控制信号的同步输出。
result: 在挑战性的闭环基准测试中显著超越了现有方法，在指令遵循、时空理解和罕见场景处理方面表现出明显优势。
conclusion: 证明了将多模态理解与生成式建模相结合是构建更具泛化性和鲁棒性的具身智能决策系统的有效途径。
---

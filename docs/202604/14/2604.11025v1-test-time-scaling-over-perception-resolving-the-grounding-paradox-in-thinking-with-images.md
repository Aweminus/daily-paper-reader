...
tags: ["query:vlm-rl"]
score: 8.0
evidence: 用于细粒度视觉推理和感知的测试时缩放
tldr: 针对多模态大模型在细粒度视觉推理中面临的“定位悖论”（即在获取证据前难以确定观察位置），本文提出TTSP框架，将感知视为可扩展的推理过程。该方法通过生成多条探索性感知路径、基于熵的置信度过滤及迭代优化，显著提升了模型在复杂视觉任务中的鲁棒性，为多模态推理提供了高效的感知缩放方案。
selection_source: fresh_fetch
motivation: 现有多模态模型在细粒度推理时面临“定位悖论”，即在未获得关键证据前难以准确决定观察位置，导致推理过程脆弱。
method: 提出TTSP框架，通过生成多条探索性感知轨迹、利用熵进行置信度评估并迭代细化观察，将感知转化为可扩展的推理过程。
result: 实验表明TTSP在多个高分辨率和通用多模态基准测试中均优于强基线，且展现出优异的可扩展性和Token效率。
conclusion: 在推理阶段对感知进行缩放是解决感知不确定性并实现鲁棒多模态推理的一种极具前景的方向。
---

## 摘要
最近的多模态大语言模型（MLLMs）已开始支持通过在推理过程中调用缩放和裁剪等视觉工具来实现“图像思维”。然而，这些系统在细粒度视觉推理方面仍然表现脆弱，因为它们必须在获得正确决策所需的证据之前，就决定该观察哪里。我们将这种循环依赖识别为“定位悖论”（Grounding Paradox）。为解决这一问题，我们提出了感知层面的测试时缩放（TTSP），这是一个将感知本身视为可扩展推理过程的框架。TTSP 生成多个探索性感知轨迹，利用基于熵的置信度估计过滤不可靠的轨迹，将验证后的观察结果提炼为结构化知识，并针对未解决的不确定性迭代优化后续探索。在高分辨率和通用多模态推理基准上的广泛实验表明，TTSP 在各种骨干模型规模下均一致优于强基准模型，同时展现出良好的可扩展性和 Token 效率。我们的结果表明，在测试时缩放感知是解决感知不确定性下鲁棒多模态推理的一个极具前景的方向。

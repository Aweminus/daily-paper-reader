...
motivation: 现有心理理论评估多集中于文本输入，缺乏对纯视觉场景下多模态大模型推理能力的关注与可解释性研究。
method: 提出 VisionToM 框架，利用干预向量将视觉表示与语义目标对齐，通过引导模型注意力来减少对虚假语言先验的依赖。
result: 在 EgoToM 视频基准测试及开放式生成任务中，该方法显著增强了模型推断他人心理状态的准确性。
conclusion: 通过视觉干预增强多模态大模型的 ToM 能力，是实现更可靠、更具对齐性的人机协作的关键一步。
---

...
tags: ["query:vlm-rl"]
score: 8.0
evidence: 用于迭代推理的闭环VLM智能体
tldr: 本研究针对视觉语言模型（VLM）在3D理解及6D物体位姿推断方面的不足，提出了一种基于闭环VLM智能体的方法。该方法通过“观察-评估-更新-渲染”的迭代推理过程，结合多视图推理、物体中心坐标系可视化和单轴旋转预测等推理技术，在无需微调的情况下显著提升了文本引导的6D位姿重排性能，并成功应用于机器人抓取任务，展示了VLM在复杂空间推理中的潜力。
selection_source: fresh_fetch
motivation: 现有的视觉语言模型在根据文本指令推断3D场景中物体的目标6D位姿时，往往难以实现精确的空间理解。
method: 提出一种闭环迭代推理框架，利用VLM作为智能体进行“观察-评估-更新”循环，并结合多视图选择与坐标系可视化技术。
result: 在无需额外微调的情况下，该方法在6D位姿预测任务上显著优于现有模型，并有效提升了机器人操作的成功率。
conclusion: 研究证明通过推理时优化和闭环反馈，通用VLM能够克服3D理解瓶颈，实现高精度的文本引导物体位姿重排。
---

## 摘要
视觉语言模型（VLM）展现出强大的视觉推理能力，但在 3D 理解方面仍面临挑战。特别是，VLM 往往无法推断出 3D 场景中目标物体与文本描述一致的目标 6D 位姿。然而，我们发现通过一些推理时技术和迭代推理，VLM 可以获得显著的性能提升。具体而言，给定一个由 RGB-D 图像（或 3D 网格组成的复合场景）表示的 3D 场景，以及一条指定所需状态变化的文本指令，我们重复以下循环：观察当前场景；评估其是否符合指令；为目标物体提出位姿更新；应用更新；并渲染更新后的场景。通过这种闭环交互，VLM 实际上充当了一个智能体。我们进一步引入了对该闭环过程至关重要的三种推理时技术：(i) 带有辅助视角选择的多视角推理，(ii) 以物体为中心的坐标系可视化，以及 (iii) 单轴旋转预测。在无需任何额外微调或新模块的情况下，我们的方法在预测目标物体的文本引导目标 6D 位姿方面超越了现有方法。该方法在闭源和开源 VLM 上均表现出一致的有效性。此外，将我们的 6D 位姿预测与简单的机器人运动规划相结合时，它比现有方法能实现更成功的机器人操纵。最后，我们进行了消融实验，以证明所提各项技术的必要性。

...
tldr: 针对多模态学习中标注成本高且模态重要性随训练动态变化的问题，本文提出了一种基于强化学习的主动学习框架RL-MBA。该框架通过自适应模态贡献平衡（AMCB）动态调整模态权重，并利用证据融合（EFDA）评估样本难度以优先选择高信息量样本。实验证明，RL-MBA在多个数据集上显著提升了分类准确率和模态公平性，有效解决了传统方法在融合阶段规则固定、无法感知训练动态的问题。
selection_source: fresh_fetch
motivation: 传统多模态主动学习方法通常假设模态重要性是固定的，忽略了训练过程中模态贡献的动态变化以及样本难度的差异。
method: 提出RL-MBA框架，将样本选择建模为马尔可夫决策过程，通过强化学习动态平衡模态贡献并结合证据融合进行难度感知的策略调整。
result: 在Food101、KineticsSound和VGGSound等数据集上的实验表明，RL-MBA在有限标注预算下显著优于现有强基准方法。
conclusion: RL-MBA通过动态适应模态价值和样本难度，为多模态主动学习提供了一种更高效、更公平的样本选择方案。
---

## 摘要
多模态学习整合了来自图像、文本和音频等不同模态的互补信息以提升模型性能，但其成功依赖于大规模标注数据，而获取这些数据的成本十分昂贵。主动学习（AL）通过选择性地标注具有信息量的样本来缓解这一挑战。在多模态设置中，许多方法隐含地假设模态重要性在各轮次间是稳定的，并在融合阶段保持固定的选择规则，这使得它们对多模态学习的动态特性不敏感，即模态的相对价值和实例难度会随着训练的进行而发生偏移。为了解决这一问题，我们提出了 RL-MBA，这是一个用于模态平衡、难度感知的多模态主动学习强化学习框架。RL-MBA 将样本选择建模为马尔可夫决策过程，其策略能够自适应模态贡献、不确定性和多样性，而奖励机制则鼓励准确率的提升与平衡。两个关键组件驱动了这种适应性：(1) 自适应模态贡献平衡（AMCB），通过强化反馈动态调整模态权重；(2) 用于难度感知策略调整的证据融合（EFDA），通过基于不确定性的证据融合来估计样本难度，从而优先选择具有信息量的样本。在 Food101、KineticsSound 和 VGGSound 上的实验表明，RL-MBA 持续优于强基准方法，在有限的标注预算下同时提升了分类准确率和模态公平性。

...
## 摘要
语义世界模型使具身智能体能够超越纯几何表示，对物体、关系和空间上下文进行推理。在有机计算（Organic Computing）中，此类模型是在不确定性和资源约束下实现目标驱动自适应的关键驱动因素。核心挑战在于如何在有限的动作预算内获取观测值，从而最大化模型质量和下游实用性。语义场景图（SSG）为此目的提供了一种结构化且紧凑的表示。然而，在有限的动作时界内构建这些图需要探索策略在信息增益与导航成本之间进行权衡，并判断何时额外的动作会产生收益递减。本研究提出了一种用于具身语义场景图生成的模块化导航组件，并通过替换策略优化方法和重新审视离散动作表述，使其决策过程现代化。我们研究了紧凑且更细粒度的、更大的离散运动集，并将针对原子动作的单头策略与针对动作分量的分解多头策略进行了比较。我们评估了课程学习和可选的基于深度的碰撞监督，并评估了 SSG 的完整性、执行安全性以及导航行为。结果表明，在相同的奖励塑造下，仅替换优化算法就能使 SSG 完整性相对于基准提高 21%。深度信息主要影响执行安全性（无碰撞运动），而完整性基本保持不变。将现代优化方法与更细粒度的分解动作表示相结合，可以在完整性与效率之间取得最佳的整体权衡。

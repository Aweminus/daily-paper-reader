...
date: 2026-04-02
pdf: "https://arxiv.org/pdf/2604.01848v2"
tags: ["query:vlm-rl"]
score: 6.0
evidence: 研究视觉语言模型的视觉不变性和几何推理
tldr: 本研究探讨了多模态大模型（VLM）在基础几何变换下的脆弱性。尽管VLM在语义识别上表现出色，但在处理旋转、缩放等变换时缺乏鲁棒的空间不变性。通过对符号草图、自然照片和抽象艺术的系统评估，发现当语义信息稀疏时，模型性能大幅下降。研究揭示了当前VLM在语义理解与空间推理之间存在系统性差距，强调了增强几何基础的重要性。
selection_source: fresh_fetch
motivation: 探究当前多模态大模型在处理基础几何变换（如旋转和缩放）时是否具备稳健的空间不变性与等变性。
method: 在符号草图、自然图像和抽象艺术等多个视觉领域，对不同架构和规模的VLM进行系统性的几何变换评估。
result: 实验表明VLM在语义内容稀疏时性能骤降，且在不同模型架构和提示策略下均表现出对几何变换的极度敏感。
conclusion: 当前VLM在语义理解与空间推理之间存在显著鸿沟，未来的多模态系统需要更强的几何感知能力。
---

...
motivation: 现有的3D推理方法通常依赖特定领域微调或固定的手工流程，限制了其在处理复杂场景时的灵活性和泛化能力。
method: 提出由规划、定位和代码智能体组成的协作框架，通过任务分解、自由形式定位和可执行程序实现精确的几何推理。
result: 在多个具有挑战性的3D理解基准测试中，MAG-3D在无需额外训练的情况下取得了最先进的性能表现。
conclusion: MAG-3D证明了通过多智能体动态协作和代码化推理，可以有效提升大模型在复杂3D场景下的零样本定位与理解能力。
---

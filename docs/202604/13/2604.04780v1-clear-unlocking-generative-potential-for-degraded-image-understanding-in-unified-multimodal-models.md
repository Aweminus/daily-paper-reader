...
tags: ["query:vlm-rl"]
score: 6.0
evidence: 在统一多模态模型中连接视觉理解与推理，处理退化图像。
tldr: 针对现实场景中图像退化损害多模态理解的问题，本文提出 CLEAR 框架。该框架通过监督微调建立“先生成后回答”的推理模式，利用潜空间表示桥接实现生成与推理的直接优化，并采用交替 GRPO 强化学习算法，在回答正确性奖励下协同优化文本推理与视觉生成。实验证明，CLEAR 在不损失清晰图像性能的前提下，显著提升了模型对退化图像的理解鲁棒性。
selection_source: fresh_fetch
motivation: 现有的统一多模态模型虽具备生成能力，但在处理退化图像时未能有效利用其生成路径来辅助理解推理。
method: 提出包含退化感知微调、潜空间表示桥接以及交替 GRPO 强化学习的三阶段框架，实现生成与理解任务的深度耦合。
result: 在 MMD-Bench 评测中，CLEAR 显著增强了模型对退化图像的鲁棒性，且发现任务驱动的优化能自发提升中间视觉质量。
conclusion: 通过将生成能力融入推理过程，可以有效解锁统一多模态模型在处理低质量视觉输入时的潜力。
---

## 摘要
模糊、噪声、压缩和光照不良导致的图像退化严重损害了现实场景中的多模态理解。将理解与生成结合在单一架构中的统一多模态模型是应对这一挑战的自然选择，因为其生成路径可以对被退化破坏的细粒度视觉结构进行建模。然而，这些模型在处理退化输入时未能利用自身的生成能力。我们将这种脱节归因于两个复合因素：现有的训练方案从未要求模型在推理过程中调用生成，且标准的“解码-再编码”路径不支持有效的联合优化。我们提出了 CLEAR，这是一个通过三个渐进步骤连接这两种能力的框架：(1) 在退化感知数据集上进行监督微调，以建立“先生成后回答”的推理模式；(2) 潜表征桥接（Latent Representation Bridge），用生成与推理之间的直接、可优化的连接取代“解码-再编码”的绕路；(3) 交替 GRPO（Interleaved GRPO），这是一种在答案正确性奖励下联合优化文本推理和视觉生成的强化学习方法。我们构建了 MMD-Bench，涵盖了六个标准多模态基准测试中的三个退化严重程度等级。实验表明，CLEAR 在保持清晰图像性能的同时，显著提高了对退化输入的鲁棒性。我们的分析进一步揭示，移除像素级重建监督会产生具有更高感知质量的中间视觉状态，这表明任务驱动的优化与视觉质量是自然对齐的。

...
tags: ["query:vlm-rl"]
score: 8.0
evidence: 基于决策的视觉问答智能体
tldr: 针对知识库视觉问答（KB-VQA）中固定检索流程难以适应多样化问题且检索与推理脱节的问题，本文提出一种基于决策的搜索智能体框架。该方法将KB-VQA建模为多步决策过程，智能体可根据当前状态自主选择回答、图像检索、文本检索或生成描述等操作。通过自动收集推理轨迹进行微调，该方法在InfoSeek和E-VQA数据集上达到了SOTA性能，显著提升了模型处理长尾事实的能力。
selection_source: fresh_fetch
motivation: 传统的检索增强生成方法采用固定的顺序流程，导致检索与推理分离，难以灵活应对复杂的知识问答需求。
...
conclusion: 将KB-VQA转化为动态决策过程能有效对齐检索证据与问题需求，是提升知识密集型视觉问答任务性能的有效途径。
---

...
tldr: 针对表格推理中纯符号方法缺乏视觉感知的问题，本文提出CFMS框架。该框架采用由粗到细的两阶段模式：首先利用多模态大模型生成包含多维知识的推理图谱，随后由符号引擎基于该图谱执行精确的迭代运算。实验证明，CFMS在WikiTQ和TabFact上表现优异，尤其在处理大型表格和使用小型骨干模型时展现出极强的鲁棒性。
selection_source: fresh_fetch
motivation: 传统的表格推理方法主要依赖符号逻辑，缺乏对表格整体视觉模式的感知，限制了对复杂半结构化数据的理解。
method: 提出一种两阶段框架，先利用多模态大模型合成高层视觉感知的知识元组，再引导符号引擎进行细粒度的迭代推理。
result: 在WikiTQ和TabFact基准测试中取得了竞争性的准确率，并在处理大规模表格和小型骨干模型时表现出显著的鲁棒性。
conclusion: CFMS通过解耦视觉感知与符号推理，有效提升了模型在复杂表格任务中的推理效率和泛化能力。
---

## 摘要
对表格数据进行推理是问答和事实验证等任务的关键能力，因为它要求模型能够理解自由形式的问题和半结构化的表格。然而，尽管思维链（CoT）等方法引入了推理链，但纯符号方法由于无法感知整体视觉模式，在本质上存在局限性。为了解决这一问题，我们提出了“由粗到细多模态综合框架”（CFMS），这是一种新颖的两阶段范式，在层次上将高层视觉感知与细粒度符号推理进行解耦。在粗粒度阶段，CFMS 利用多模态大语言模型（MLLM）对多视角知识元组进行一次性综合。该元组随后作为动态推理图谱来指导细粒度阶段，在此阶段，符号引擎对表格执行有针对性且高效的迭代操作序列。在 WikiTQ 和 TabFact 基准测试上的广泛实验表明，CFMS 达到了具有竞争力的准确率。该框架在处理大型表格以及使用较小骨干模型实例化时表现出显著的鲁棒性，验证了其有效性和泛化能力。

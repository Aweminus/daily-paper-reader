...
method: "将RGB-D图像划分为N*N网格并提取语义、颜色和深度信息，转化为结构化文本作为PLM的输入。"
result: 在R2R和RxR基准测试及实地部署中，该方法显著降低了模型参数量和对训练数据的依赖。
conclusion: SOL-Nav通过将视觉观测语言化，成功利用预训练语言模型的推理能力实现了高效且泛化性强的导航。
---

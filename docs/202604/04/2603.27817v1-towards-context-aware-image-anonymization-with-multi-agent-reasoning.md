...
tldr: 针对街景图像中个人身份信息（PII）匿名化过度处理或遗漏的问题，本文提出CAIAMAR框架。该框架结合多智能体推理与扩散模型，通过PDCA循环和空间上下文感知，实现对直接和间接标识符的精准分割与匿名化。在显著降低重识别风险的同时，保持了极高的图像质量和下游任务性能，且支持本地部署，符合GDPR透明度要求。
selection_source: fresh_fetch
motivation: 现有匿名化方法在处理间接标识符时存在过度处理或遗漏，且依赖云端API存在数据主权风险。
method: 提出一种基于多智能体推理的PDCA循环框架，结合空间感知检测、开集分割及外观去相关的扩散生成技术。
result: 9.1）并保留了语义分割性能。
conclusion: 该框架通过多智能体协作实现了上下文感知的精准匿名化，为符合隐私法规的图像处理提供了高效、可解释的本地化方案。
---

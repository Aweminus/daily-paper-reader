...
tldr: 本研究针对具身智能中指令粒度控制不足的问题，提出了Mini-BEHAVIOR-Gran基准测试，通过为同一任务提供从高层目标到详细步骤的多级指令，系统研究了指令粒度对智能体行为的影响。研究发现，规划宽度是衡量粒度的最佳指标，并揭示了指令粒度与智能体性能之间呈非单调的U型关系，即在极细和极粗粒度下表现较好，而中间粒度表现较差。
...
## 摘要
指令粒度是语言引导的具身人工智能（Embodied AI）中一个重要但缺乏有效控制的变量。现有的基准测试通常为每个任务配对单一的静态指令，这使得研究当同一任务以不同细节程度描述时，智能体行为如何变化变得十分困难。我们推出了 Mini-BEHAVIOR-Gran，这是一个用于指令粒度受控研究的新基准，它扩展了 Mini-BEHAVIOR，为每个任务提供了多种指令变体，涵盖了从高层目标描述到逐步引导的各种粒度。利用该基准，我们比较了四种用于跨任务粒度量化的候选指标：Token 数量、实体数量、动作动词数量以及规划宽度（planning-width），并发现宽度与智能体性能的相关性最为一致。使用宽度来组织训练和评估进一步揭示了指令粒度与性能之间存在非单调的 U 型关系，在极细和极粗的两个极端均出现峰值。进一步的分析表明，粗粒度下的性能回升与浅层对齐（shallow grounding）有关，即智能体学习到了以视觉为主导的策略。

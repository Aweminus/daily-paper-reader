...
tldr: 针对智能体搜索中检索系统固定导致的性能瓶颈，本文提出CoSearch框架。该框架利用GRPO算法首次实现了推理智能体与生成式文档排序模型的联合训练。通过引入语义分组策略解决排序模型输入多变的问题，并设计结合排序质量与轨迹反馈的复合奖励。实验证明，这种联合优化机制在多个问答基准上显著优于现有方法，有效提升了复杂问题的检索与回答能力。
selection_source: fresh_fetch
motivation: 现有研究将检索系统视为固定工具，实验表明检索能力的局限已成为提升智能体搜索性能的核心瓶颈。
method: 采用GRPO算法联合训练推理智能体与排序模型，并结合语义分组策略与复合奖励机制来优化学习过程。
result: 在七个单跳和多跳问答基准测试中，CoSearch 均表现出优于强基准模型的性能，验证了各设计组件的有效性。
conclusion: 联合训练推理与检索系统是可行且高效的，是构建高性能搜索智能体的关键要素。
---

## 摘要
智能体搜索（Agentic search）——即训练能够通过迭代推理、发起查询并综合检索信息来回答复杂问题的智能体——在强化学习（RL）的推动下取得了显著进展。然而，现有方法（如 Search-R1）将检索系统视为固定工具，仅优化推理智能体，而检索组件保持不变。初步实验表明，在七个问答（QA）基准测试中，理想检索器（oracle）与固定检索系统之间的差距在相对 F1 指标上高达 +26.8%，这表明检索系统是扩展智能体搜索性能的关键瓶颈。受此发现启发，我们提出了 CoSearch，这是一个通过群体相对策略优化（GRPO）联合训练多步推理智能体和生成式文档排序模型的框架。为了对排序器进行有效的 GRPO 训练（其输入随推理轨迹而变化），我们引入了一种语义分组策略，通过词元级（token-level）相似度对子查询进行聚类，从而在不增加额外采样（rollouts）的情况下形成有效的优化组。我们进一步设计了一种复合奖励，将排序质量信号与轨迹级结果反馈相结合，为排序器提供即时和长期的学习信号。在七个单跳和多跳问答基准测试上的实验表明，该方法相较于强基准模型有持续的提升，消融实验验证了各项设计选择的有效性。我们的结果表明，推理智能体与检索系统的联合训练既可行又具有强大的性能，为未来的搜索智能体指明了一个关键要素。

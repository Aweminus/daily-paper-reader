...
tags: ["query:vlm-rl"]
score: 6.0
evidence: 优化表格问答的多模态推理
tldr: TABQAWORLD是一个针对多轮表格问答的免训练多模态推理框架。针对现有方法在表格序列化中存在的表示误差累积及推理成本高的问题，该框架通过动态切换视觉与文本表示的动作条件选择策略，并结合表格元数据优化推理轨迹与压缩低复杂度动作。实验表明，该方法在提升推理准确率的同时显著降低了推理延迟，为高效可靠的表格推理建立了新标准。
selection_source: fresh_fetch
motivation: 现有多轮表格推理方法依赖固定文本序列化，导致表示误差随轮次累积，且现有的表格定位方法推理成本过高。
method: 提出一种动作条件多模态选择策略动态切换表征，并利用表格元数据优化推理轨迹及压缩低复杂度动作。
result: "在基准测试中实现了SOTA性能，相比基线准确率提升4.87%，且在静态设置下准确率提升5.42%并降低了33.35%的延迟。"
conclusion: TABQAWORLD通过联合优化表征与估计，在无需训练的情况下实现了高效且可靠的多轮表格推理。
---

## 摘要
多模态推理已成为增强推理模型推理能力的强大框架。虽然多轮表格推理方法通过工具使用和奖励建模提高了推理准确性，但它们依赖于固定的文本序列化来进行表格状态读取。这在表格编码中引入了表示误差，且该误差会在多轮对话中显著累积。表格定位方法虽然可以缓解这种累积，但以推理计算和成本为代价，使得实际部署变得不切实际。为解决这一问题，我们推出了 TABQAWORLD，这是一个通过表示和估计联合优化表格动作的表格推理框架。在表示方面，TABQAWORLD 采用了一种动作条件多模态选择策略，该策略在视觉和文本表示之间动态切换，以最大化表格状态读取的可靠性。在估计方面，TABQAWORLD 通过包括维度、数据类型和键值在内的表格元数据优化逐步推理轨迹，安全地规划轨迹并压缩低复杂度动作，从而减少对话轮数和延迟。作为一个无需训练的框架，实证评估表明 TABQAWORLD 达到了最先进的性能，准确率比基准提高了 4.87%，且相比静态设置实现了 5.42% 的准确率提升和 33.35% 的推理延迟降低，为可靠且高效的表格推理建立了新标准。

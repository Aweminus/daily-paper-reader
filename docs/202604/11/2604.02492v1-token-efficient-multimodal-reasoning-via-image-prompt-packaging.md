...
tags: ["query:vlm-rl"]
score: 6.0
evidence: 通过图像提示封装进行多模态推理
tldr: "本研究针对多模态大模型推理成本高的问题，提出“图像提示封装（IPPg）”范式，通过将结构化文本直接嵌入图像来减少文本Token开销。在GPT-4和Claude等模型上的实验表明，该方法可降低35.8%-91.0%的推理成本，且在特定任务中保持甚至提升了准确率。研究还揭示了视觉编码选择对模型性能的显著影响，为多模态系统设计提供了新思路。"
...
## 摘要
大规模部署大型多模态语言模型受限于基于 Token 的推理成本，然而视觉提示策略的成本-性能表现尚未得到充分表征。我们引入了图像提示封装（Image Prompt Packaging, IPPg），这是一种将结构化文本直接嵌入图像以减少文本 Token 开销的提示范式，并在五个数据集、三个前沿模型（GPT-4.1、GPT-4o、Claude 3.5 Sonnet）以及两类任务（VQA 和代码生成）上对其进行了基准测试。我们推导了一个按 Token 类型分解节省成本的计算公式，并展示了 IPPg 实现了 35.8% 至 91.0% 的推理成本降低。尽管 Token 压缩率高达 96%，但在许多设置下准确率仍具竞争力，不过结果高度依赖于模型和任务：GPT-4.1 在 CoSQL 上同时实现了准确率和成本的提升，而 Claude 3.5 在多个 VQA 基准测试中反而导致了成本增加。系统的错误分析产生了一个失败模式分类法：空间推理、非英语输入和字符敏感操作最容易受到影响，而模式结构化任务受益最大。一项包含 125 种配置的渲染消融实验揭示了 10 到 30 个百分点的准确率波动，从而确立了视觉编码选择是多模态系统设计中的一级变量。

...
tags: ["query:vlm-rl"]
score: 6.0
evidence: 4D场景的查询时推理与语义落地
tldr: 针对动态4D场景理解中语义推理不一致的问题，本文提出PanopticQuery框架。该框架结合4D高斯泼溅技术进行高保真重建，并通过多视图语义共识机制，将2D语义预测在时空维度上进行聚合与优化，实现了从噪声视图到全局一致4D语义的转化。此外，研究还推出了Panoptic-L4D基准测试，证明了该方法在处理复杂语言查询（如动作、关系和交互）方面的卓越性能。
selection_source: fresh_fetch
motivation: 现有的4D重建方法在处理复杂语义推理（如物体交互和时空关系）时，难以将多视图的噪声预测转化为全局一致的理解。
method: 提出一种基于4D高斯泼溅的框架，利用多视图语义共识机制和神经场优化，将2D语义预测提升并整合为结构化的4D语义基元。
result: 在提出的Panoptic-L4D基准测试中，该方法在处理涉及属性、动作和空间关系的复杂语言查询方面达到了最先进水平。
conclusion: PanopticQuery通过统一的查询时推理，有效解决了动态场景中的语义一致性问题，为4D环境的自然语言理解提供了强有力的工具。
---

## 摘要
通过自然语言查询理解动态 4D 环境，不仅需要准确的场景重建，还需要在空间、时间和视角上实现鲁棒的语义对齐。尽管近期使用神经表示的方法推动了 4D 重建的发展，但它们在上下文推理方面仍存在局限，特别是在处理交互、时序动作和空间关系等复杂语义时。一个关键挑战在于如何将噪声较多且依赖视角的预测转化为全局一致的 4D 解释。我们提出了 PanopticQuery，这是一个用于 4D 场景统一查询时推理的框架。我们的方法基于 4D 高斯泼溅（4D Gaussian Splatting）进行高保真动态重建，并引入了一种多视角语义共识机制，通过聚合多个视角和时间帧的 2D 语义预测，实现自然语言查询的对齐。该过程过滤了不一致的输出，增强了几何一致性，并通过神经场优化将 2D 语义提升为结构化的 4D 对齐结果。为了支持评估，我们提出了 Panoptic-L4D，这是一个用于动态场景中基于语言查询的新基准。实验表明，PanopticQuery 在复杂语言查询方面达到了新的最先进水平，能够有效处理属性、动作、空间关系和多物体交互。补充材料中提供了视频演示。

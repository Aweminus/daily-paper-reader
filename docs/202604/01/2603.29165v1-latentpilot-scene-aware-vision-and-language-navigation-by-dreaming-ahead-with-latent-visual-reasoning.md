...
tags: ["query:vlm-rl"]
score: 8.0
evidence: 用于导航决策的潜空间视觉推理
tldr: 本文提出LatentPilot，旨在解决现有视觉语言导航模型忽视动作对未来视觉影响的问题。受人类预见未来能力的启发，该模型在训练阶段利用未来观测学习动作条件下的视觉动力学，推理时则无需未来信息。通过飞轮式训练机制和跨步传递的视觉潜变量，智能体能够“预见”动作后果并进行潜空间推理。实验证明该方法在R2R-CE等多个基准测试中达到SOTA，并在真实机器人部署中展现出极强的环境感知与决策能力。
selection_source: fresh_fetch
motivation: 现有模型主要依赖历史和当前观测，缺乏对动作与视觉环境变化之间因果关系的理解，限制了决策的鲁棒性。
method: 采用飞轮式训练机制结合跨步传递的视觉潜变量，使智能体在训练中学习动作驱动的视觉动态并实现“预见未来”的推理。
result: 在R2R-CE、RxR-CE和R2R-PE等多个主流基准测试中均取得SOTA性能，并在真实机器人环境验证中表现优异。
conclusion: 通过在潜空间中模拟未来视觉变化，LatentPilot显著增强了智能体对环境-动作动态关系的理解和导航决策能力。
---

## 摘要
现有的视觉语言导航（VLN）模型主要基于过去和当前的视觉观测进行推理，而在很大程度上忽略了由动作引起的未来视觉动态。因此，它们往往缺乏对动作与视觉世界变化之间因果关系的有效理解，限制了决策的鲁棒性。相比之下，人类可以利用动作-动态因果关系来想象近期未来，从而提高对环境的理解和导航选择。受此启发，我们提出了 LatentPilot，这是一种新的范式，它在训练期间利用未来观测作为有价值的数据源来学习以动作为条件的视觉动态，而在推理时无需访问未来帧。具体而言，我们提出了一种飞轮式训练机制，通过迭代收集同策略（on-policy）轨迹并重新训练模型，以更好地匹配智能体的行为分布，并在智能体偏离过大时触发专家接管。LatentPilot 进一步在没有显式监督的情况下学习视觉隐标记（latent tokens）；这些隐标记在连续隐空间中进行全局注意力计算，并跨步传递，既作为当前输出又作为下一步输入，从而使智能体能够预见未来并推理动作将如何影响后续观测。在 R2R-CE、RxR-CE 和 R2R-PE 基准测试上的实验达到了新的 SOTA 结果，在不同环境下的真实机器人测试证明了 LatentPilot 对场景中环境-动作动态的卓越理解。项目页面：https://abdd.top/latentpilot/

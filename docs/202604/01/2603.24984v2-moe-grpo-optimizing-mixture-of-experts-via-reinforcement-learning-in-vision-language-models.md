...
tags: ["query:vlm-rl"]
score: 6.0
evidence: 利用强化学习优化VLM架构
tldr: 针对视觉语言模型（VLM）中混合专家（MoE）架构存在的确定性Top-K路由导致专家过拟合及选择次优的问题，本文提出MoE-GRPO框架。该框架利用强化学习中的组相对策略优化（GRPO）将专家选择建模为序列决策，通过奖励反馈实现自适应路由。同时引入模态感知引导以提升训练稳定性。实验证明，该方法在多模态基准测试中优于传统路由机制，实现了更优的专家多样性和任务级专业化。
selection_source: fresh_fetch
motivation: 传统的确定性Top-K路由机制限制了专家组合的最优性，且容易导致专家过拟合。
method: 提出基于强化学习（GRPO）的路由优化框架，并结合模态感知引导来增强训练稳定性和效率。
result: 在多模态图像和视频基准测试中，MoE-GRPO显著优于标准Top-K路由及其变体。
conclusion: 通过强化学习优化路由策略能有效提升MoE模型的多样性与专业化水平，是优化多模态模型性能的有效途径。
---

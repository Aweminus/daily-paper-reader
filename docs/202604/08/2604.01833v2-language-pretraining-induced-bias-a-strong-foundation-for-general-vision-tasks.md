...
tags: ["query:vlm-rl"]
score: 6.0
evidence: 将LLM参数与视觉任务对齐以进行多模态推理
...
## 摘要
语言预训练模型与视觉预训练模型中离群参数的比例存在显著差异，这使得跨模态（语言与视觉）迁移本质上比跨领域适配更具挑战性。因此，许多先前的研究侧重于跨领域迁移，而非尝试桥接语言与视觉模态，并假设由于参数空间迥异，语言预训练模型不适用于下游视觉任务。与此假设相反，我们表明，添加一个桥接训练阶段作为模态适配学习器，可以有效地将大语言模型（LLM）参数与视觉任务对齐。具体而言，我们提出

...
motivation: 现有的VLA模型在处理复杂家居环境中的全局布局、精细几何和高维连续动作时表现不佳。
method: 提出一种空间感知的VLA框架，通过多模态输入增强和协同训练辅助解码器来强化模型对空间和操作的表征能力。
result: 在拾取、放置、开关等家庭重排任务中，该方法显著优于传统的直接模仿学习，表现出更强的空间理解能力。
conclusion: 结合辅助任务和多模态学习的空间感知增强是提升通用家政机器人VLA模型性能的有效途径。
---

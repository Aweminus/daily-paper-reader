...
## 摘要
视觉语言模型 (VLMs) 在不同视角下对同一物体的描述往往不一致，这阻碍了具身智能体随时间构建一致语义表示的能力。先前的方法通过离线多视图聚合或将探索、数据关联和描述学习解耦的多阶段流水线来解决不一致性，但对先前观察到的物体的推理能力有限。在本文中，我们提出了一种统一的记忆增强型视觉语言智能体，它在单个自回归框架内同时处理数据关联、物体描述和探索策略。该模型处理当前的 RGB 观测、自顶向下的探索地图以及序列化为物体级标记 (tokens) 的物体级情节记忆，从而确保在长序列中保持持久的物体身份和语义一致性。为了以自监督的方式训练模型，我们利用基于分歧的策略和伪描述模型在逼真的 3D 环境中收集了一个数据集，该模型强制要求多视图描述历史之间的一致性。在手动标注的物体级测试集上进行的广泛评估表明，与基准模型相比，标准描述评分提高了多达 +11.86%，描述自相似性提高了 +7.39%，同时通过紧凑的场景表示实现了可扩展的性能。代码、模型权重和数据可在 https://hsp-iit.github.io/epos-vlm/ 获取。

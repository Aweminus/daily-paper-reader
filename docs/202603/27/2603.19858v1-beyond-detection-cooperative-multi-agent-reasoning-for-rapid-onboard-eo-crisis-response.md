...
tldr: 针对当前地球观测（EO）任务中地面处理延迟高的问题，本文提出一种分层多智能体架构，用于星上快速危机响应。该系统通过协调预警、领域分析和决策智能体，结合视觉语言模型与遥感工具，实现了多模态观测的协同推理。在边缘计算平台上的实验证明，该方法在降低计算开销的同时保证了决策准确性，为未来自主卫星星座提供了可行方案。
selection_source: fresh_fetch
motivation: 传统的地面中心化地球观测流程因下行带宽和计算延迟，难以满足灾害响应的实时性需求。
method: 提出一种由预警、领域分析和决策智能体组成的分层多智能体架构，利用视觉语言模型和专业工具进行星上协同推理。
result: 在野火和洪水监测场景下的实验表明，该路由式流水线显著降低了计算开销并保持了连贯的决策输出。
conclusion: 该研究验证了分布式多智能体推理在资源受限的星上环境中的可行性，为下一代自主地球观测任务奠定了基础。
---

## 摘要
快速识别危险事件对于支持灾害响应的下一代地球观测（EO）任务至关重要。然而，目前的监测流程在很大程度上仍以地面为中心，由于下行链路限制、多源数据融合约束以及详尽场景分析的计算成本，导致了显著延迟。本研究提出了一种分层多智能体架构，用于在严格的资源和带宽约束下进行星上地球观测处理。该系统通过在事件驱动的决策流程中协调专门的 AI 智能体，实现了对互补多模态观测数据的利用。AI 智能体可以部署在分布式环境中的多个节点上，例如卫星平台。预警智能体根据星上观测生成快速假设，并选择性地激活特定领域的分析智能体，而决策智能体则整合证据以发布最终警报。该架构结合了视觉语言模型、传统遥感分析工具和角色专业化智能体，在最小化不必要计算的同时，实现了对多模态观测的结构化推理。利用代表性卫星数据，在目前已部署在轨的边缘计算平台工程模型上执行了概念验证实现。在野火和洪水监测场景下的实验表明，所提出的基于路由的流程在保持连贯决策输出的同时，显著降低了计算开销，证明了分布式智能体推理在未来自主地球观测星座中的可行性。

...
## 摘要
多模态自动程序修复（MAPR）通过要求模型对源代码、文本问题描述以及 GUI 截图等视觉产物进行联合推理，扩展了传统的程序修复。尽管最近基于大语言模型（LLM）的修复系统取得了显著成果，但现有方法仍面临若干局限：僵化的工作流管道限制了调试过程中的探索，视觉推理通常在全页面截图上进行而缺乏局部定位，且失败的修复尝试很少被转化为可重用的知识。为了应对这些挑战，我们提出了 FailureMem，这是一个集成三种关键机制的多模态修复框架：一种平衡了结构化定位与灵活推理的混合工作流-智能体架构；能够实现区域级视觉定位的主动感知工具；以及一个将过去的修复尝试转化为可重用指导的故障记忆库。在 SWE-bench Multimodal 上的实验表明，FailureMem 相比 GUIRepair 将解决率提高了 3.7%。

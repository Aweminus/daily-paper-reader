...
tldr: 针对现有视觉语言导航（VLN）依赖冗长指令且缺乏全局空间先验的问题，本文提出了FloorPlan-VLN新范式。该范式利用结构化语义平面图作为全局空间先验，使智能体仅需简洁指令即可导航。为此，研究者构建了包含10k+片段的大规模数据集，并提出了FP-Nav方法，通过双视图对齐和辅助推理任务实现观测、平面图与指令的对齐。实验证明该方法在导航成功率上显著优于现有基准，并具备良好的鲁棒性。
selection_source: carryover_cache
motivation: 现有VLN任务过度依赖详细指令而忽视了平面图等全局空间先验，限制了智能体对空间结构的理解能力。
method: 提出了FP-Nav方法，利用双视图时空对齐视频序列和辅助推理任务，将实时观测、语义平面图与简洁指令进行多模态对齐。
result: "在FloorPlan-VLN基准测试中，FP-Nav相比现有最先进的VLN基准方法在导航成功率上实现了超过60%的相对提升。"
conclusion: FloorPlan-VLN证明了平面图引导导航的有效性，为开发具备更强空间智能的导航系统提供了新方向。
---

## 摘要
现有的视觉语言导航（VLN）任务要求智能体遵循冗长的指令，忽略了一些潜在有用的全局空间先验，从而限制了它们对空间结构的推理能力。尽管人类可读的空间示意图（如平面图）在现实世界的建筑中无处不在，但目前的智能体缺乏理解和利用这些图表的认知能力。为了弥补这一差距，我们引入了 FloorPlan-VLN，这是一种利用结构化语义平面图作为全局空间先验的新范式，使得仅凭简洁指令即可进行导航。我们首先构建了 FloorPlan-VLN 数据集，该数据集包含跨越 72 个场景的 1 万多个回合。它将 100 多张带有语义标注的平面图与基于 Matterport3D 的导航轨迹以及省略了逐步引导的简洁指令进行配对。随后，我们提出了一种简单而有效的方法 FP-Nav，该方法利用双视图、时空对齐的视频序列以及辅助推理任务来对齐观测结果、平面图和指令。在这一新基准下进行评估时，我们的方法显著优于经过适配的最先进 VLN 基准模型，在导航成功率上实现了超过 60% 的相对提升。此外，全面的噪声建模和真实世界部署证明了 FP-Nav 对执行漂移和平面图失真的可行性与鲁棒性。这些结果验证了平面图引导导航的有效性，并凸显了 FloorPlan-VLN 是迈向更具空间智能导航的有前景的一步。

...
tags: ["query:vlm-rl"]
score: 7.0
evidence: 针对大语言模型推理智能体的可验证奖励强化学习
tldr: SAGE是一个多智能体自我进化框架，旨在减少对大规模人工标注数据的依赖，提升LLM的长程推理能力。该框架通过挑战者、规划者、执行者和评论者四个角色协同工作，实现从少量种子数据出发的闭环自我进化。实验证明，SAGE在数学和代码生成任务上显著提升了模型性能，为大模型推理能力的持续进化提供了新路径。
selection_source: fresh_fetch
motivation: 现有的强化学习推理方法过度依赖人工标注，且自博弈方法在长程多步推理中缺乏显式规划和质量控制。
method: 提出由挑战者生成任务、规划者制定步骤、执行者求解、评论者筛选质量的四智能体闭环自我进化框架。
result: "在Qwen-2.5-7B模型上，SAGE使LiveCodeBench和OlympiadBench的准确率分别提升了8.9%和10.7%。"
conclusion: SAGE通过多智能体协同和显式规划，实现了稳定且高效的推理能力自我进化，显著增强了模型在复杂任务上的表现。
---

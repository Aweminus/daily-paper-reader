...
tldr: 针对视觉语言导航（VLN）中全局先验地图与局部环境动态变化不匹配的问题，本文提出了HaltNav框架。该框架利用轻量级的文本拓扑图（osmAG）进行全局规划，并结合多模态大模型（MLLM）实现局部指令执行与障碍感知。其核心贡献在于“反应式视觉停止（RVH）”机制，能实时检测环境变化并触发重新规划，同时通过生成式数据增强提升了系统的鲁棒性。
selection_source: fresh_fetch
motivation: 传统的VLN依赖沉重的度量地图且难以应对现实中如关门或拥挤等局部路径变化导致的执行失败。
method: 提出HaltNav分层框架，利用osmAG进行全局导航，并通过基于MLLM的RVH机制实时检测障碍并动态更新拓扑图以重新规划。
result: 实验证明，该框架在无需繁琐指令的情况下优于基准方法，并显著提升了长程导航在环境变化下的鲁棒性。
conclusion: HaltNav通过结合轻量级拓扑先验与反应式视觉感知，为实现稳健、自主的视觉语言导航提供了一种高效的层次化解决方案。
---

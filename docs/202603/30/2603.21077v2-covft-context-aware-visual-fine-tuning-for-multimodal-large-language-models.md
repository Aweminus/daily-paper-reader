...
tldr: 针对多模态大模型中视觉编码器是否应微调的争议，本文指出传统微调方法因忽略上下文导致的视觉偏好冲突而表现不稳定。为此，提出了上下文感知视觉微调（CoVFT）框架，通过引入上下文向量提取和上下文混合专家模块，实现了视觉特征的动态适配。实验证明，CoVFT在12个基准测试中表现卓越且稳定，甚至使7B模型性能超越了13B模型，揭示了视觉编码器优化的巨大潜力。
selection_source: fresh_fetch
motivation: 现有视觉微调方法因忽略多模态上下文导致的视觉偏好冲突，无法在各项任务中稳定超越冻结基准。
method: 提出CoVFT框架，通过上下文向量提取（CVE）和上下文混合专家（CoMoE）模块，将多模态上下文显式引入视觉适配过程。
result: 在12个多模态基准测试中达到SOTA性能，且微调后的7B模型表现优于其13B版本的平均水平。
conclusion: 视觉编码器的上下文感知优化是提升多模态大模型性能的关键，具有显著的性能提升潜力。
---

...
tldr: 针对大型视觉语言模型（LVLM）在多轮对话中因视觉标记过多导致推理成本高的问题，本文提出了一种名为 MetaCompress 的学习型提示无关标记缩减方法。该方法将标记缩减建模为可学习的压缩映射，统一了剪枝与合并操作，并通过高效的训练范式优化压缩策略。实验证明，MetaCompress 在多轮视觉问答任务中实现了优异的效率与准确率平衡，具有良好的泛化能力。
selection_source: fresh_fetch
motivation: 现有的标记缩减方法主要针对单轮对话，在多轮对话中容易丢失后续问题所需的视觉信息，且启发式策略性能有限。
method: 提出 MetaCompress，将标记缩减形式化为可学习的压缩映射，并引入一种数据高效的训练范式来学习最优的剪枝与合并策略。
result: 在多个多轮视觉问答基准测试和 LVLM 架构上，MetaCompress 均取得了优于现有方法的效率与准确率权衡。
conclusion: 学习型的提示无关压缩方法能有效解决多轮对话中的视觉信息保留问题，是提升 LVLM 推理效率的有效途径。
---

...
tldr: 针对视频推理分割中动态场景下轨迹感知不足的问题，本文提出TrajSeg框架。该框架基于多模态大模型，引入双向文本-轨迹对齐机制（涵盖定位与描述任务），显著增强了模型对物体轨迹的理解。通过帧级内容集成模块和统一掩码解码器，实现了从轨迹到掩码的高效转换与端到端训练。实验表明，TrajSeg在多个视频分割基准测试中均达到SOTA水平。
selection_source: fresh_fetch
motivation: 现有视频推理分割方法在处理剧烈动态场景时，因单向且隐式的文本-轨迹对齐导致轨迹感知能力不足。
method: 提出TrajSeg框架，通过双向文本-轨迹对齐增强感知，并利用帧级内容集成模块与统一掩码解码器实现端到端分割。
result: 在多个引用和推理视频分割数据集上，TrajSeg在所有指标上均超越了现有的视频推理分割方法。
conclusion: 该研究证明了双向对齐和统一架构在提升多模态大模型视频轨迹感知与分割性能方面的有效性。
---

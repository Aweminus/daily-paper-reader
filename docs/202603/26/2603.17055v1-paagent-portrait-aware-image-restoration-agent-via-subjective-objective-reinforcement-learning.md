...
tldr: 针对现有图像修复代理缺乏经验总结机制导致工具搜索效率低的问题，本文提出PaAgent。该代理通过构建自进化的肖像库并结合检索增强生成（RAG）技术，实现了对修复工具的精准选择。同时，引入主客观强化学习策略，结合图像质量评分与语义洞察，显著提升了在复杂及非均匀退化场景下的感知能力。实验证明PaAgent在多种退化场景下均表现优异。
selection_source: carryover_cache
motivation: 现有图像修复代理缺乏对过去交互经验的总结机制，导致在选择最优修复工具时搜索效率低下。
method: 提出包含自进化肖像库、RAG工具选择机制以及结合主客观评分的强化学习策略的PaAgent框架。
result: 在涵盖单退化和混合退化场景的8个基准测试中，PaAgent均展现出超越现有方法的修复性能。
conclusion: PaAgent通过经验总结与多维度强化学习，有效解决了复杂场景下的自动化图像修复难题。
---

...
tldr: 针对连续环境下的零样本视觉语言导航（VLN-CE）难题，本文提出EmergeNav框架。该框架认为VLM在导航中的瓶颈在于缺乏组织指令遵循和感知落地的执行结构。EmergeNav通过“计划-解决-转换”层级结构、目标条件感知提取、对比双记忆推理及双视野感知，将导航建模为结构化具身推理。在无需特定任务训练或显式地图的情况下，该方法在VLN-CE基准上取得了显著的零样本性能，证明了执行结构对提升VLM导航稳定性的重要性。
selection_source: carryover_cache
motivation: 现有的视觉语言模型在连续导航任务中虽有语义先验，但因缺乏有效的执行结构而难以实现稳定的长程指令遵循。
method: 提出EmergeNav框架，结合层级化执行结构、目标感知提取、双记忆推理和双视野感知，将导航过程转化为结构化的具身推理。
result: "在VLN-CE任务中，仅使用开源VLM且无需额外训练或地图，Qwen3-VL-32B模型达到了37.00%的成功率。"
conclusion: 显式的执行结构是释放VLM先验知识并将其转化为稳定具身导航行为的关键因素。
---

## 摘要
连续环境下的零样本视觉语言导航（VLN-CE）对于现代视觉语言模型（VLM）而言仍然具有挑战性。尽管这些模型编码了有用的语义先验，但其开放式推理并不能直接转化为稳定的长程具身执行。我们认为，关键瓶颈不仅在于知识的缺失，更在于缺乏一种用于组织指令遵循、感知落地、时间进度和阶段验证的执行结构。我们提出了 EmergeNav，这是一个将连续 VLN 形式化为结构化具身推理的零样本框架。EmergeNav 结合了用于阶段结构化执行的“计划-解决-转换”（Plan--Solve--Transition）层级结构、用于目标条件感知提取的 GIPE、用于进度落地的对比双记忆推理，以及用于时间对齐局部控制和边界验证的角色分离双视野（Dual-FOV）传感。在 VLN-CE 任务上，EmergeNav 仅使用开源 VLM 骨干网络，在无需任务特定训练、显式地图、图搜索或航路点预测器的情况下，实现了强大的零样本性能，在 Qwen3-VL-8B 上达到 30.00 SR，在 Qwen3-VL-32B 上达到 37.00 SR。这些结果表明，显式的执行结构是将 VLM 先验转化为稳定具身导航行为的关键要素。

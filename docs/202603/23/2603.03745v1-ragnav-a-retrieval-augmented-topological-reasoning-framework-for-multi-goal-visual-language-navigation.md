...
tags: ["query:vlm-rl"]
score: 8.0
evidence: 具有拓扑推理能力的视觉语言导航智能体
tldr: 针对多目标视觉语言导航（VLN）中存在的空间幻觉和规划漂移问题，本文提出了RAGNav框架。该框架通过双基存储系统结合底层拓扑地图与高层语义森林，实现了物理连接与语义抽象的融合。利用锚点引导检索和拓扑邻域评分传播机制，RAGNav显著提升了目标间的可达性推理和序列规划效率，在复杂多目标导航任务中达到了SOTA性能。
selection_source: carryover_cache
motivation: 传统RAG范式在处理多目标关联时缺乏显式空间建模，容易导致空间幻觉和导航规划失效。
method: 提出双基存储系统，结合拓扑地图与语义森林，并引入锚点引导检索和拓扑邻域评分传播机制进行语义校准。
result: 实验结果表明，RAGNav在复杂的多目标导航任务中显著增强了推理能力，并取得了当前最先进的性能表现。
conclusion: 通过将语义推理与物理结构相结合，RAGNav有效解决了多目标VLN中的空间建模难题，提升了导航的准确性与效率。
---

## 摘要
视觉语言导航（VLN）正从单点寻路演进为更具挑战性的多目标视觉语言导航。该任务要求智能体在准确识别多个实体的同时，协同推理它们的空间物理约束和顺序执行次序。然而，由于缺乏显式的空间建模，通用的检索增强生成（RAG）范式在处理多对象关联时，往往会遭受空间幻觉和规划漂移的影响。为了应对这些挑战，我们提出了 RAGNav，这是一个弥合语义推理与物理结构之间鸿沟的框架。RAGNav 的核心是一个双基准记忆（Dual-Basis Memory）系统，它集成了一个用于维持物理连通性的底层拓扑图和一个用于分层环境抽象的高层语义森林。基于这一表示，该框架引入了锚点引导的条件检索和拓扑邻域得分传播机制。该方法促进了候选目标的快速筛选和语义噪声的消除，同时利用拓扑邻域中固有的物理关联进行语义校准。这一机制显著增强了目标间可达性推理能力和顺序规划的效率。实验结果表明，RAGNav 在复杂的多目标导航任务中实现了最先进（SOTA）的性能。

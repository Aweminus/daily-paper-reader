...
tags: ["query:vlm-rl"]
score: 7.0
evidence: 理解多模态大模型推理过程中的感知受损
tldr: 本研究探讨了多模态大模型在长链推理中出现的感知受损现象，发现其根源在于注意力分散导致模型“失去焦点”。为此，作者提出了VRGA框架，这是一种无需训练的方法，通过熵聚焦准则识别关键视觉注意力头并进行重权引导。该方法在多个基准测试中显著提升了模型的视觉定位和推理精度，为理解和优化多模态推理过程提供了新视角。
selection_source: fresh_fetch
motivation: 多模态大模型在进行复杂推理时，视觉注意力往往会偏离与问题相关的区域，导致感知能力随推理深度增加而下降。
method: 提出一种无需训练的视觉区域引导注意力（VRGA）框架，利用熵聚焦准则筛选视觉头并重权其注意力分布。
result: 实验表明，VRGA有效缓解了感知退化问题，在视觉定位和推理准确性上均取得了显著提升。
conclusion: 该研究揭示了推理与视觉注意力分散的关联，证明了通过引导注意力聚焦可以增强多模态模型在深度推理中的感知稳定性。
---

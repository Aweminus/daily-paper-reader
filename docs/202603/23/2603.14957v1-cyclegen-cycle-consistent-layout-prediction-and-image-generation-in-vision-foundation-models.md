...
title_zh: CyCLeGen：视觉基础模型中的循环一致布局预测与图像生成
...
tags: ["query:vlm-rl"]
score: 6.0
evidence: 统一视觉语言模型中通过强化学习实现的自我改进
...
## 摘要
我们提出了 CyCLeGen，这是一种统一的视觉-语言基础模型，能够在单一自回归框架内同时进行图像理解和图像生成。与依赖独立感知和合成模块的现有视觉模型不同，CyCLeGen 采用了一种完全集成的架构，通过“图像->布局->图像”和“布局->图像->布局”的生成循环来强制执行循环一致性学习。这种统一的表述带来了两个关键优势：内省（introspection），使模型能够对其自身的生成结果进行推理；以及数据效率，允许在循环一致性引导的强化学习目标下，通过合成监督实现自我提升。大量实验表明，CyCLeGen 在多种图像理解和生成基准测试中均取得了显著提升，凸显了统一视觉-语言基础模型的潜力。

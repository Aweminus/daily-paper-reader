...
tags: ["query:vlm-rl"]
score: 6.0
evidence: MLLM中多模态推理的视觉微调分析
tldr: 针对多模态大语言模型（MLLM）中视觉编码器微调效果不稳定的问题，本文指出其根源在于忽略上下文导致的视觉偏好冲突。为此，研究者提出了上下文感知视觉微调框架（CoVFT），通过引入上下文向量提取和上下文混合专家模块，将多模态语境融入视觉适配过程。实验证明，CoVFT在12个基准测试中表现卓越且稳定，使7B模型性能超越了13B模型，充分挖掘了视觉编码器优化的潜力。
selection_source: fresh_fetch
motivation: 现有视觉微调方法因忽略多模态上下文导致参数更新方向冲突，难以在各类任务中稳定超越冻结视觉编码器的基准。
method: 提出CoVFT框架，通过上下文向量提取（CVE）和上下文混合专家（CoMoE）模块，将多模态上下文显式引入视觉适配过程。
result: 在12个多模态基准测试中取得SOTA性能，且微调后的7B模型平均表现优于参数量更大的13B模型。
conclusion: 显式引入多模态上下文是解决视觉微调冲突、提升MLLM视觉编码器优化效率和稳定性的关键途径。
---

...
tldr: 针对开放世界中自主智能体难以应对新颖物体的问题，本文提出一种神经符号架构。该架构结合了符号规划、强化学习（RL）和大型语言模型（LLM）。利用LLM的常识推理能力识别缺失的算子，辅助符号规划器生成计划，并编写奖励函数引导RL学习新算子的控制策略。实验证明该方法在机器人领域的算子发现与学习方面优于现有技术。

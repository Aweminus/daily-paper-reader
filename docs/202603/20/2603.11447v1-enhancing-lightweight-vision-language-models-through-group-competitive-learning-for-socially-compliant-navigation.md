...
method: 提出了群体竞争学习（GCL）策略，结合群体竞争目标（GCO）和非对称群体优化（AGO）来增强模型性能。
result: "实验显示GCL使3B规模的模型在F1分数上提升了40%，甚至超越了参数量更大的8B基准模型。"
conclusion: GCL策略成功实现了高精度与高计算效率的平衡，为轻量级模型在实时社交导航任务中的应用提供了有效途径。
---

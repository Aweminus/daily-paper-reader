...
tldr: 针对多模态大模型在隐空间视觉推理中因硬对齐目标导致探索性受限的问题，本文提出CoLVR框架。该框架通过基于角度扰动的对比学习目标扩展语义隐空间，并结合隐式轨迹对比奖励进行强化学习微调，旨在优化推理过程并促进多样化推理行为。实验表明，CoLVR在VSP、Jigsaw及MMStar等基准测试上显著提升了模型的推理能力和泛化性能。
selection_source: fresh_fetch
motivation: 现有的隐空间视觉推理方法过度依赖硬对齐目标，限制了模型在推理过程中的探索能力和语义表达。
method: 提出CoLVR框架，利用角度扰动引导的对比学习扩展隐空间，并引入隐式轨迹对比奖励进行强化学习优化。
result: "在VSP和Jigsaw任务上分别提升了5.83%和8.00%，在MMStar等域外基准测试中也表现出优异的泛化能力。"
conclusion: CoLVR通过对比优化显著增强了隐空间视觉推理的探索性，为提升多模态大模型的复杂推理能力提供了有效路径。
---

## 摘要
由于潜空间视觉推理在探索性推理方面的潜力，近期的研究倾向于使多模态大语言模型（MLLMs）通过传播连续隐藏状态而非将中间步骤解码为离散标记来执行视觉推理。然而，现有工作通常依赖于硬对齐目标，强制潜空间表示与预定义的视觉特征匹配，从而严重限制了潜空间推理过程的探索性。为了解决这一问题，我们提出了 CoLVR（潜空间视觉推理的对比优化）。为了获得更具探索性的视觉推理，CoLVR 引入了一个潜空间对比训练框架。首先，CoLVR 通过基于角度扰动引导的潜空间对比目标学习多样化且具探索性的表示，这扩展了语义潜空间并避免了过度约束的嵌入。随后，CoLVR 在强化学习（RL）后训练中采用了潜空间轨迹对比奖励，以实现对潜空间视觉推理过程的细粒度优化，从而培养多样化的推理行为。实验表明，CoLVR 显著增强了潜空间表示的探索能力，在 VSP 上平均提升了 5.83%，在 Jigsaw 上提升了 8.00%，同时在域外基准测试中也优于现有的潜空间模型，在 MMStar 上获得了 3.40% 的增益。数据、代码和模型已发布在 https://github.com/Oscar-dzy/CoLVR。

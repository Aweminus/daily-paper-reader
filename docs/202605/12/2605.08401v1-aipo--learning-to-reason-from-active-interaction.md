...
tldr: 针对大语言模型在强化学习中受限于自身能力边界的问题，本文提出AIPO框架。该框架通过在训练探索阶段引入主动多智能体交互，允许模型在遇到推理瓶颈时咨询验证、知识和推理智能体，从而获取细粒度引导并扩展能力边界。实验证明，AIPO在AIME、MATH500等多个推理基准上显著提升了性能，且训练后的模型可独立推理。
selection_source: fresh_fetch
motivation: 现有的强化学习算法受限于模型自身能力边界，且依赖外部专家演示的方法效率低下且空间受限。
method: 提出AIPO框架，使策略模型在训练中能主动咨询验证、知识和推理智能体，并结合重要性采样与裁剪策略优化学习过程。
result: 在AIME、MATH500、GPQA-Diamond和LiveCodeBench等多个推理基准上，AIPO一致提升了模型的推理性能和泛化能力。
conclusion: AIPO通过主动交互有效地扩展了策略模型的推理能力边界，是一种提升大语言模型推理能力的稳健且高效的强化学习框架。
---

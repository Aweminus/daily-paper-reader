...
tags: ["query:vlm-rl"]
score: 7.0
evidence: 使用视觉上下文进行定位的GUI智能体
tldr: 针对GUI智能体在处理可访问性树时面临的冗余严重及缺乏空间结构信息等问题，本文提出了A11y-Compressor框架。该框架通过Compressed-a11y流水线，利用模态检测、冗余缩减和语义结构化技术，将冗长的线性化树转换为紧凑且富有结构信息的表示形式。实验表明，该方法在显著降低Token消耗的同时，有效提升了智能体在复杂GUI任务中的成功率。
selection_source: fresh_fetch
motivation: 现有的GUI可访问性树表示法存在严重的文本冗余，且缺乏描述元素间空间关系的结构化信息。
method: 提出A11y-Compressor框架，通过模态检测、冗余消除和语义结构化重组，将线性化树转化为紧凑的结构化表示。
result: "在OSWorld基准测试中，该方法将输入Token数压缩至原来的22%，并使任务平均成功率提升了5.1个百分点。"
conclusion: 通过优化GUI观测值的表示方式，可以在大幅提升计算效率的同时增强AI智能体的任务执行能力。
---

## 摘要
与图形用户界面 (GUI) 交互的 AI 智能体需要有效的观测表示以实现可靠的接地 (grounding)。无障碍树 (accessibility tree) 是一种常用的基于文本的格式，用于编码 UI 元素属性，但它存在冗余，且缺乏诸如元素间空间关系等结构化信息。我们提出了 A11y-Compressor，这是一个将线性化无障碍树转换为紧凑且结构化表示的框架。我们的实现版本 Compressed-a11y 采用了一个轻量级且结构化的转换流水线，包含模态检测、冗余减少和语义结构化。在 OSWorld 基准测试上的实验表明，Compressed-a11y 将输入 token 数量减少至原始大小的 22%，同时将任务成功率平均提高了 5.1 个百分点。

...
tags: ["query:vlm-rl"]
score: 6.0
evidence: 用于导航的 LLM 编排智能体视频生成
tldr: "本研究提出 Action Agent，一个将代理式视频生成与流约束扩散控制相结合的两阶段机器人导航框架。第一阶段利用大语言模型编排视频扩散模型，生成物理合理的导航视频，将生成成功率从 35% 提升至 86%；第二阶段通过 FlowDiT 模型将目标视频转化为连续速度指令。该方法在人形机器人、无人机和轮式机器人上表现出色，实现了轨迹想象与执行的解耦，显著提升了跨具身导航的泛化能力和成功率。"
selection_source: fresh_fetch
motivation: 旨在解决多具身机器人导航中，如何将高层语言指令有效转化为物理可行的轨迹想象与精确控制指令的问题。
method: 采用大语言模型驱动的视频生成阶段来规划视觉轨迹，并结合基于流约束的扩散 Transformer 将视频特征转化为具体的运动控制参数。
result: "在仿真中实现了 73.2% 的导航成功率，并在真实人形机器人上达到了 64.7% 的任务完成率，同时保持高效的运行频率。"
conclusion: 该研究证明了将轨迹想象与动作执行解耦是实现可扩展、具身感知且语言引导的机器人导航的有效范式。
---

## 摘要
我们提出了 Action Agent，这是一个两阶段框架，将智能体导航视频生成与流约束扩散控制相结合，用于多具身机器人导航。在第一阶段，大语言模型 (LLM) 作为编排模块，负责选择视频扩散模型、通过迭代验证优化提示词，并积累跨任务记忆，从而根据语言和图像输入合成符合物理规律的第一人称导航视频。这使得 50 个导航任务中的视频生成成功率从 35%（单次尝试）提升至 86%。在第二阶段，我们引入了 FlowDiT，这是一种流约束扩散 Transformer (Flow-Constrained Diffusion Transformer)，它利用动作空间去噪扩散将优化的目标视频和语言指令转换为连续的速度指令。FlowDiT 集成了 DINOv2 视觉特征、用于自我运动表示的学习光流，以及用于语义停止的 CLIP 语言嵌入。我们在 RECON 户外导航数据集上进行预训练，并在 Isaac Sim 中采集的 203 个 Unitree G1 人形机器人片段上进行微调，以校准速度动力学。一个仅含 43M 参数的检查点在仿真中实现了 73.2% 的导航成功率，在真实 Unitree G1 机器人的未知室内环境中，开环执行下的任务完成率达到 64.7%，运行频率为 40-47 Hz。我们在三种具身形态上评估了 Action Agent：Unitree G1 人形机器人（真实硬件）、无人机和轮式移动机器人（Isaac Sim），证明了将轨迹想象与执行解耦为语言引导导航提供了一种可扩展且具身感知的范式。

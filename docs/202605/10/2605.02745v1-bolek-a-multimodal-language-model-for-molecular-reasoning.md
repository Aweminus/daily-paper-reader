...
tldr: 本研究提出Bolek，一种紧凑的多模态语言模型，旨在解决分子属性预测模型缺乏可解释性或解释与结构脱节的问题。通过将Morgan指纹嵌入注入指令微调的文本解码器，并在分子对齐和基于思维链的推理任务上进行微调，Bolek在15项TDC分类任务中显著优于基座模型和更大规模的TxGemma。其生成的解释高度关联分子特征，具备极强的可审计性和泛化能力。
selection_source: fresh_fetch
motivation: 传统的分子预测模型缺乏解释，而现有语言模型的解释往往与分子结构关联性弱，难以审计。
method: 将Morgan指纹嵌入注入Qwen3-4B解码器，并利用分子描述、RDKit指标及合成思维链进行多任务微调。
result: Bolek在多数分类任务上超越了规模更大的TxGemma，且其引用的数值指标与RDKit计算结果高度一致。
conclusion: 针对性的模态注入和基于可验证特征的推理监督，能有效提升紧凑型模型在分子推理中的准确性与可审计性。
---

## 摘要
分子属性模型越来越多地支持高风险的药物发现决策，但其输出往往难以审计：经典预测器返回没有依据的分数，而语言模型虽然能生成流畅的解释，但与输入分子的关联性较弱。我们介绍了 Bolek，这是一种紧凑的多模态语言模型，通过将 Morgan 指纹嵌入注入到经过指令微调的文本解码器中，使自然语言推理立足于分子结构。Bolek 在分子对齐任务（包括分子描述、RDKit 描述符预测和子结构检测）以及基于 15 个 TDC 二分类任务的下游推理上进行了微调，使用了锚定在具体分子特征上的合成思维链。在这些任务中，Bolek 在“是/否”模式下的所有终点以及思维链模式下 15 个终点中的 13 个上都优于其 Qwen3-4B-Instruct 基座模型，将平均 ROC/PR AUC 从 0.55 提高到 0.76。尽管其规模不到 TxGemma-9B-Chat 的一半，但它在 15 个二分类任务中的 13 个上也优于后者。Bolek 的解释比基准大语言模型更具依据：它在每个思维链中引用数值描述符的频率高出 10-100 倍，且引用的数值与 RDKit 在 TPSA、MolLogP 和 MolWt 等关键描述符上高度一致（Spearman rho = 0.87-0.91）。泛化能力扩展到了训练面板之外：在 15 个未见的 TDC 分类终点上，Bolek 在 5 个终点上与 TxGemma 持平；尽管在训练期间从未见过下游回归任务，但它在 3 个预留的回归终点上产生了显著的秩相关性。这些结果表明，针对性的模态注入和与可验证分子特征挂钩的推理监督可以产生紧凑且可审计的分子推理模型。

...
tldr: 针对自主地球观测（EO）智能体在复杂任务规划中面临的推理错误和组合复杂性挑战，本文提出了轻量级多模态元规划器（LMMP）框架。该框架通过双重感知机制结合图像特征与任务语义，并引入元任务库注入专家知识。通过两阶段训练优化，LMMP显著提升了工具调用准确率和任务成功率，且具有极强的通用性，可作为插件提升多种执行器性能。
selection_source: fresh_fetch
motivation: 现有的单模型架构在处理动态地球观测任务时，难以应对复杂的推理需求和物理可行性约束。
method: 提出LMMP框架，利用双重感知机制和元任务库标准化领域逻辑，并通过监督微调与直接偏好优化进行两阶段训练。
result: 在EarthBench和ThinkGeo数据集上的实验表明，该框架显著提高了工具调用精度和任务成功率。
conclusion: LMMP为地球观测智能体提供了一种高效、可扩展的规划方案，展现了强大的即插即用性能和跨任务泛化能力。
---

## 摘要
自主地球观测（EO）智能体正从被动感知转向复杂的多步任务执行。然而，当前将规划与执行集成在单一模型中的架构，在动态 EO 场景中往往难以应对组合复杂性和推理错误。为解决这些挑战，我们提出了轻量级多模态元规划器（LMMP）框架。LMMP 包含一种双重感知机制，将战略规划建立在多模态图像特征和高层任务语义的基础之上。关键在于，我们引入了一个元任务库（Meta Task Library），将遥感专家知识直接注入工作流，从而规范了领域逻辑并确保规划在物理上可行。我们进一步实现了一个两阶段训练流水线，通过专家蒸馏的有监督微调初始化元规划器，并根据执行反馈通过直接偏好优化对其进行精炼。在源自 EarthBench 和 ThinkGeo 的数据集上进行的广泛实验表明，LMMP 显著提高了工具调用准确率和任务成功率。此外，该框架表现出强大的“即插即用”通用性，在先前未见的 EO 任务中持续增强了各种执行器骨干网络的性能。

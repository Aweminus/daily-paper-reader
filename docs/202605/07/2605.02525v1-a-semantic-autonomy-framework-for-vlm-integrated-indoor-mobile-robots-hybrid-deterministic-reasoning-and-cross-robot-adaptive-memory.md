...
tldr: "针对室内移动机器人难以理解自然语言指令及视觉语言模型（VLM）推理延迟高、缺乏记忆的问题，本文提出了一种语义自主框架。该框架结合了混合确定性推理与跨机器人自适应记忆，通过七步参数解析器处理了88%的指令，显著降低了延迟。实验证明，该系统在无GPU的树莓派平台上实现了极高的语义解析准确率和跨机器人知识迁移，大幅提升了机器人的实用性。"
selection_source: fresh_fetch
motivation: 现有的机器人导航框架缺乏语义理解能力，而集成VLM虽能解决语义问题，但面临推理延迟高和会话间遗忘的挑战。
method: 提出六层语义自主栈，利用混合确定性推理处理简单指令，并通过五类语义存储框架实现跨会话学习和跨机器人知识共享。
result: "在树莓派5上实现了88%的指令在0.1毫秒内处理，延迟降低了10.3万倍，且语义解析和迁移准确率均达到100%。"
conclusion: 该框架证明了在低功耗边缘设备上实现高效、具备记忆能力且可协作的语义自主导航是切实可行的。
---

## 摘要
自主室内移动机器人可以使用 ROS 2 Navigation 2 等成熟框架可靠地导航到度量坐标，但它们缺乏解释表达意图而非位置的自然语言指令的能力。视觉语言模型（VLM）提供了弥合这一差距所需的语义推理，但其推理延迟（在消费级硬件上每次决策需 2-9 秒）和逐会话遗忘限制了实际部署。本文提出了语义自主栈（Semantic Autonomy Stack），这是一个用于语义自主室内导航的六层参考框架，并在配备现成边缘硬件的物理机器人上验证了一个包含混合确定性-VLM 推理和跨机器人自适应记忆的完整实例。一个七步参数解析器在不调用语言模型、摄像头或 GPU 的情况下，在 0.1 毫秒内处理了 88% 的指令；只有真正模糊的指令才会升级到 VLM 推理。一个具有明确范围分类（全局环境知识、各操作员偏好、各机器人能力）的五类语义记忆框架实现了跨会话学习和跨机器人知识迁移：通过在一台机器人上进行 VLM 交互学习到的偏好被提升为确定性解析，并通过共享的编译摘要传输到第二台机器人，实现了实测 103,000 倍的延迟降低。在两台定制的差分驱动机器人上进行的 82 次场景级决策和三个会话的实验验证表明，语义迁移准确率达到 100%（33/33，95% 置信区间 [0.894, 1.000]），语义解析准确率达到 100%，且具备并发多机器人运行的可行性——所有这些均在无板载 GPU 的树莓派 5（Raspberry Pi 5）平台上实现，且无需任何训练数据。

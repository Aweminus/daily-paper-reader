...
tldr: 针对大模型智能体在处理超长上下文环境（如大规模代码库）时面临的记忆退化和搜索低效问题，本文提出了“上下文收集决策过程（CGDP）”框架，将其形式化为部分可观测马尔可夫决策过程（POMDP）。研究者通过引入基于谓词的显式信念状态和程序化穷尽门控，替代了LLM的隐式搜索逻辑。实验表明，该方法在提升多跳推理准确率的同时，显著降低了Token消耗，为构建高效、模块化的智能体搜索系统提供了理论指导。
selection_source: fresh_fetch
motivation: LLM智能体在处理超出上下文窗口的大规模数据时，常因隐式记忆退化导致重复搜索或过早停止。
method: 将智能体搜索建模为CGDP框架下的近似汤普森采样，并引入基于谓词的持久化信念状态与程序化穷尽检测机制。
result: "在多个问答领域中，该方法将多跳推理能力提升了11.4%，并节省了高达39%的Token使用量。"
conclusion: 将智能体循环视为CGDP过程，有助于设计出更模块化、更高效且不干扰原有逻辑的智能体搜索增强方案。
---

## 摘要
大语言模型 (LLM) 智能体被部署在复杂的环境中——例如海量代码库、企业数据库和对话历史——在这些环境中，相关状态远超其上下文窗口。为了在这些空间中导航，智能体必须迭代地探索环境以寻找相关信息。然而，在缺乏显式基础设施的情况下，智能体的工作记忆可能会退化为搜索状态的有损表示，从而导致冗余工作（如重复循环）和过早停止。在这项工作中，我们将这一挑战形式化为上下文收集决策过程 (CGDP)，这是一种专门的部分可观测马尔可夫决策过程 (POMDP)，其中智能体的目标是自适应地细化其信念状态，以隔离任务所需的必要信息。我们将 LLM 的行为建模为该 CGDP 中的近似汤普森采样 (Thompson Sampling)，并引入了一种基于谓词的方法，将 LLM 的隐式搜索分解为显式且模块化的操作。随后，我们为迭代式 LLM 智能体推导出两种即插即用的干预措施：一种持久的、基于谓词的信念状态，它在限制上下文的同时保留了多跳推理能力；以及一个程序化的穷尽门控，用于在不提前停止的情况下终止无成效的搜索。在四种方法和三个问答领域中，我们通过实验验证了用基于 CGDP 的信念状态替换 LLM 的隐式状态，可将多跳推理能力提高多达 11.4%；而模块化的程序化穷尽检测在不降低智能体性能的情况下，节省了多达 39% 的 token。最终，我们认为将 LLM 智能体循环框架化为 CGDP 可以指导代理式搜索工具链中模块化、无干扰改进的设计。

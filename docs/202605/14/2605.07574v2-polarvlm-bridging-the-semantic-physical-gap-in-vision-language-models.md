...
date: 2026-05-08
pdf: "https://arxiv.org/pdf/2605.07574v2"
tags: ["query:vlm-rl"]
score: 8.0
evidence: 用于复杂物理参数推理的视觉语言模型
tldr: 针对主流视觉语言模型在处理反射和透明物体等光学歧义时的局限性，本文提出了首个集成偏振物理参数的多模态框架 PolarVLM。通过双流架构和两阶段训练策略，结合包含7.5万个指令微调对的 PolarVQA 基准，该模型成功弥合了语义与物理之间的鸿沟，在反射识别和透明物体计数等任务上显著优于传统 RGB 模型。
selection_source: fresh_fetch
motivation: 传统 RGB 图像难以区分反射和透明物体，而偏振成像能提供解决这些光学歧义的关键物理信息。
method: 提出双流架构和渐进式两阶段训练策略，将偏振物理参数融入 VLM，并构建了首个偏振感知问答基准 PolarVQA。
result: "PolarVLM 在五项评估任务中比 RGB 基线平均提升 25.4%，其中反射识别和玻璃计数分别提升 26.6% 和 34.0%。"
conclusion: PolarVLM 成功实现了物理感知的语义理解，为解决复杂光学场景下的视觉推理提供了新方向。
---
